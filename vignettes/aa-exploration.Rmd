---
title: "Extract spatial statistics within buffers around sample points : time-series data"
author: "Paul Taconet"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Extract spatial statistics within buffers around sample points : time-series data}
...
- Precipitation (daily, from [GPM](https://doi.org/10.5067/GPM/IMERGDF/DAY/06) and [TAMSAT](https://www.tamsat.org.uk/)) ;
...
- Precipitation (half-hourly, from [GPM](https://doi.org/10.5067/GPM/IMERGDF/DAY/06)); 
...
Below is the header of the final output table : 
...
```{r setup_input_param, eval=T, echo=T, results='hide'}
...
path_to_input_dates_loc<-system.file("extdata/CIV_df_dates_loc.csv", package = "getRemoteData") #file.path(path_to_working_directory,"df_hlc.csv")  
...
## For some covariates we need own data. These data are stored in a database. Here we provide the path to the DB and the names of the tables that we will use. 
...
db_table_lulc_rast<-"landcover_civ_L3" # land use / land cover raster
db_table_lulc_rat<-"landcover_civ_pixval2class"  # raster attribute table of the LU/LC table
...
db_table_builtup_rast<-"landcover_civ_builtup" # built up raster
db_table_pedology_rast<-"pedology_civ"  # pedology raster

## Useful only for the calculation of the DEM and DEM-related indices : 
...
```{r setup_packages, eval = TRUE, echo=T, results='hide'}
require(raster)
require(sp)
require(sf)
require(rgrass7)
...
extractVar_line_singleBuff<-function(network_sf,sfPoints,buffer_size){
  ...
  res<-st_join(buffer,network_sf, join = st_intersects,left = TRUE) %>% # Get the lines inside each buffer
      ...
      st_drop_geometry() %>%
      select(id,dist_line_to_pt,accumulation) %>%
      group_by(id) %>%
...
# Spatially resample a product (using bilinear resampling)
...
# qval=1 is for values that where retrieved using the LST 1km/1day products
...
```{r setup_credentials , eval=T, echo=T, results='hide'}
...
```{r setup_dates_loc, eval=T, echo=T, message = FALSE}
hlc_dates_loc<-read.csv(path_to_input_dates_loc,stringsAsFactors = F)
head(hlc_dates_loc)
```
...
```{r setup_parralel, eval=T, echo=T}
plan(multiprocess)
...
```{r modis_download, eval=F, echo=T}
...
```{r modis_lst_process, eval=F, echo=T}
...
lst1day[[1]]$var<-"lstMin1d"
lst1day[[2]]$var<-"lstMax1d"
cat("extracting MODIS LST 8 days...\n")
lst8day<-extract_modis_lst("MOD11A2.006","MYD11A2.006")
lst8day[[1]]$var<-"lstMin8d"
lst8day[[2]]$var<-"lstMax8d"
...
# Export
write.csv(lstMin1d,file.path(path_to_working_directory,"envCov_lstMin1d.csv"),row.names = F)
write.csv(lstMax1d,file.path(path_to_working_directory,"envCov_lstMax1d.csv"),row.names = F)
write.csv(lstMin8d,file.path(path_to_working_directory,"envCov_lstMin8d.csv"),row.names = F)
write.csv(lstMax8d,file.path(path_to_working_directory,"envCov_lstMax8d.csv"),row.names = F)
```
...
```{r modis_vi_process, eval=F, echo=T}
# For each date of interest, get the related time-series rasters names (i.e. the whole set of 40 days rasters)
...
# Extract the variables 
cat("Extracting ndvi...\n")
ndvi<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_VI,rasts_ndvi,"mean")
ndvi$var<-"ndvi"
cat("Extracting evi...\n")
evi<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_VI,rasts_evi,"mean")
evi$var<-"evi" 
...
# Export
write.csv(ndvi,file.path(path_to_working_directory,"envCov_ndvi.csv"),row.names = F)
write.csv(evi,file.path(path_to_working_directory,"envCov_evi.csv"),row.names = F)
```
...
```{r modis_et_process, eval=F, echo=T}
# For each date of interest, get the related time-series rasters names (i.e. the whole set of 40 days rasters)
...
# Extract
cat("Extracting et...\n")
et<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_Et,rasts_Et,"mean")
et$var<-"et"
rm(rasts_Et)
...
# Export
write.csv(et,file.path(path_to_working_directory,"envCov_et.csv"),row.names = F)
```
...
```{r gpm_daily_process, eval=F, echo=T}
# For each date of interest, get the related time-series rasters names (i.e. the whole set of 40 days rasters)
...
rain_gpmDay<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_gpmDay,rasts_gpmDay,"mean")
rain_gpmDay$var<-"rain_gpmDay"
rain_gpmDay$qval<-1
rm(rasts_gpmDay)

# Export
write.csv(rain_gpmDay,file.path(path_to_working_directory,"envCov_rain_gpmDay.csv"),row.names = F)
```   
  
#### 2.2.2/ Half-hourly precipitations

```{r gpm_hhourly_process, eval=F, echo=T}
...
rain_gpmHhour<-rain_gpmHhour %>%
  filter(!is.na(val)) %>%
  group_by(id,buffer) %>% 
  summarise(val=round(sum(val)/n()*100)) %>% 
  mutate(time_lag=0)
rain_gpmHhour$var<-"rain_gpmHhour"
rain_gpmHhour$qval<-1
rm(rasts_gpmHhour)

# Export
write.csv(rain_gpmHhour,file.path(path_to_working_directory,"envCov_rain_gpmHhour.csv"),row.names = F)
```  
...
```{r tamsat_download, eval=F, echo=T}
...
rain_tamsatDay<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_tamsat,rasts_tamsat,"mean")
rain_tamsatDay$var<-"rain_tamsatDay"
rain_tamsatDay$qval<-1
rm(rasts_tamsat)

# Export
write.csv(rain_tamsatDay,file.path(path_to_working_directory,"envCov_rain_tamsatDay.csv"),row.names = F)
``` 
...
```{r viirsdnb_download, eval=F, echo=T}
...
nightlight<-extractVar_ts(buffer_sizes,df_dates_loc,rastsNames_viirsDnb,rasts_viirsDnb,"mean")
nightlight$var<-"nightlight"
nightlight$qval<-1
rm(rasts_viirsDnb)

# Export
write.csv(nightlight,file.path(path_to_working_directory,"envCov_nightlight.csv"),row.names = F)
``` 
...
### 5.1/ Import 
```{r imcce_download, eval=F, echo=T}
...
moonMag<-hlc_dates_loc %>%
     ...
     mutate(buffer=NA) %>%
     mutate(qval=1) %>%
     dplyr::select(id,buffer,time_lag,val,var,qval)

# Export
write.csv(moonMag,file.path(path_to_working_directory,"envCov_moonMag.csv"),row.names = F)
``` 
...
wind_speed<-extractVar_ts(buffer_sizes=10,df_dates_loc,rastsNames_era5,rasts_wind_speed,"sum") # There is only 1 cell intersected (i.e. buffer_sizes=10) so the summurazing function is quite useless
wind_speed <- wind_speed %>%
  group_by(id,buffer) %>% 
  summarise(val=mean(val)) %>% 
  mutate(time_lag=0)
wind_speed$var<-"windspeed"
wind_speed$qval<-1
rm(rasts_wind_speed)
...
wind_dir<-extractVar_ts(buffer_sizes=10,df_dates_loc,rastsNames_era5,rasts_wind_dir_cardinal,"sum") # Same as above
wind_dir <- wind_dir %>%
  ...
  mutate(time_lag=0)
wind_dir$var<-"winddirection"
wind_dir$qval<-1
rm(rasts_wind_dir_cardinal)

# Export
write.csv(wind_speed,file.path(path_to_working_directory,"envCov_windspeed.csv"),row.names = F)
write.csv(wind_dir,file.path(path_to_working_directory,"envCov_winddirection.csv"),row.names = F)
```


## 7/ Elevation and elevation derivative indices with SRTM DEM data

### 7.1/ Import

Setup the path to the output datasets that will be generated

```{r dem_paths, eval=T, echo=T}
...
Create the hydrographic datasets from the DEM with the `rgrass7` package : slope, aspect, accumulation, tci, twi

```{r prepare_hydro_datasets, eval=T, echo=T}
...
```{r prepare_stream_network, eval=T, echo=T}
...
names(dem_and_derivatives_rast)<-c("elevation","slope","aspect","accumulation","tci","twi")

dem_stats<-buffer_sizes %>% # for each buffer, calculate stats
    set_names %>%
...
```{r process_stream_net, eval=T, echo=T}
# open stream network
...
# Get the accumulation value on each piece of the network. For this, take the centroid of each piece to speed up the extraction of the accumulation value over each piece
...
```{r setup_other_data, eval=T, echo=T}
...
```{r prepare_settl_pop, eval=T, echo=T}
...
## Get stats: area of the village and population density
stats_villages_1 <- villages_convexhull %>%
  ...
  mutate(village_pop_dens=population/village_area) %>% # get the population density of the village in pers/ha 
  rename(village_population = population) %>% 
  dplyr::select(codevillage,village_population,village_area,village_pop_dens) %>% 
  st_drop_geometry()
...
```{r import_builtup , eval=T, echo=T}
...
```{r process_buitup, eval=T, echo=T}
builtup_rast<-raster::raster(path_to_builtup_rast)

stats_villages_2 <- villages_convexhull %>%
  mutate(village_builtup_area=map(geom_convHull,~raster::extract(builtup_rast,as(.,'Spatial'),fun=sum,na.rm=T))) %>% # get the built-up area by intersecting the built-up raster with the convex hull polygon of each village
    mutate(village_builtup_area=as.numeric(village_builtup_area)*res(builtup_rast)[1]*res(builtup_rast)[2]) %>% # convert to m2
  st_drop_geometry()

stats_villages_3<-raster::extract(builtup_rast,spTransform(spPoints,proj4string(builtup_rast)),buffer=100,fun=sum,na.rm=T,sp=TRUE) %>% # get the built-up area around each catch point by intersecting the built-up raster with a buffer of 100 m around each catch point
  as.data.frame() %>%
  select(id,bati_raster) %>%
  mutate(bati_raster=as.numeric(bati_raster)*res(builtup_rast)[1]*res(builtup_rast)[2]) %>% # convert to m2
  set_names(c("id","var")) %>%
  mutate(val="catchpoint_builtup_area") %>%
  mutate(buffer=NA)
```
...
```{r import_pedology , eval=T, echo=T}
# we have not found the solution to read the rasters directly in the gpkg database. So we write them as tif : 
...
```{r process_pedology, eval=T, echo=T}
pedology_rast<-raster::raster(path_to_pedology_rast)
names(pedology_rast)="surf_hydro_soil"
...
pedology_stats<-buffer_sizes %>% # for each buffer, calculate stats
    set_names %>%
    map_dfr(~extractVar_rast_singleBuff(pedology_rast,spPoints,.,"sum"))

# convert surface to ha
pedology_stats <- pedology_stats %>%
  mutate(val=val*res(pedology_rast)[1]*res(pedology_rast)[2]*0.0001)
```
