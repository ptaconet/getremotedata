...
dataSources_timeSeries<-c("MOD11A1.006","MYD11A1.006","MOD11A2.006","MYD11A2.006","MOD13Q1.006","MYD13Q1.006","MOD16A2.006","MYD16A2.006","GPM_3IMERGDF","TAMSAT")
...
We setup some functions that will be used throughout the workflow to process multiple dates / times series at once.
...
# Extract a summary of a variable over 1 buffer
extractVar_singleBuff<-function(rasts,names_rasts_to_use,spPoints,buffer_size,fun_summarize){
...
rain_gpmHhour<-extractVar(buffer_sizes=10,df_dates_loc,rastsNames_gpmHhour,rasts_gpmHhour,"sum")

# Extract a summary of a variable over multiple buffers
extractVar<-function(buffer_sizes,df_dates_loc,names_rasts_to_use,rasters,fun_summarize){
  res<-buffer_sizes %>% # for each buffer, calculate stats
    ...
    future_map_dfr(~pmap_dfr(list(names_rasts_to_use,df_dates_loc$sp_points,.,fun_summarize),
                             ~extractVar_singleBuff(rasters,..1,..2,..3,..4)))  %>%
    #mutate(var=var_name) %>%
...
# Resample a product (using bilinear resampling)
...
df <- df %>% 
  ...
  mutate(qval=if_else((qval==0 & !is.na(val)),qvalue,qval))  %>% 
  select(-c(val_ahead,val_lag,val_mean))
return(df)
}
...
extract_modis_lst<-function(MOD_collection, MYD_collection){
  ...
  rasts_LstMin<-path_to_mod_myd %>%
    mutate(rast_mod=map(destfiles_mod,~getRemoteData::prepareData_modis(.,"LST_Night_1km"))) %>%
  ...
  lstMin<-extractVar(buffer_sizes,df_dates_loc,rastsNames_LST,rasts_LstMin,"mean") # to put in wide format : lstMin <- lstMin %>% unite(var,var,time_lag) %>% spread(key=var,value=val)
  lstMin$var<-"lstMin"
  cat("Extracting LstMax...\n")
  lstMax<-extractVar(buffer_sizes,df_dates_loc,rastsNames_LST,rasts_LstMax,"mean")
  lstMax$var<-"lstMax"
  #head(lstMin)
...
# Extract the variables 
cat("Extracting ndvi...\n")
ndvi<-extractVar(buffer_sizes,df_dates_loc,rastsNames_VI,rasts_ndvi,"mean")
ndvi$var<-"ndvi"
cat("Extracting evi...\n")
evi<-extractVar(buffer_sizes,df_dates_loc,rastsNames_VI,rasts_evi,"mean")
evi$var<-"evi" 
...
# Extract
cat("Extracting et...\n")
et<-extractVar(buffer_sizes,df_dates_loc,rastsNames_Et,rasts_Et,"mean")
et$var<-"et"
rm(rasts_Et)
...
rain_gpmDay<-extractVar(buffer_sizes,df_dates_loc,rastsNames_gpmDay,rasts_gpmDay,"mean")
...
# Set to 1 if there was rain, else 0 
...
rain_gpmHhour<-extractVar(buffer_sizes=10,df_dates_loc,rastsNames_gpmHhour,rasts_gpmHhour,"sum")  # With this we get if is has rain (1) or not (0) for each half hour of each night (ie the lag). This dataset will be useful to study the detailed rythms (cycles) of the mosquitoes
...
rain_tamsatDay<-extractVar(buffer_sizes,df_dates_loc,rastsNames_tamsat,rasts_tamsat,"mean")
...
Nightlight<-extractVar(buffer_sizes,df_dates_loc,rastsNames_viirsDnb,rasts_viirsDnb,"mean")
...
path_to_u_v_wind<-merge(path_to_u_wind,path_to_v_wind,by="names",suffixes = c("_u","_v"))
